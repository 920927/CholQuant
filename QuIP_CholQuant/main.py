...
@torch.no_grad()
def llama_sequential(model, dataloader, method, dev):
    print('Starting ...')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--method', type=str, default="quip", choices=['quip', 'q2n', 'cholquant']
    )
    ...
    log_filename = f"quant_{model_name}_{method_name}_{args.wbits}bit.log"
    ...
    if not args.load and args.wbits < 16:
        tick = time.time()
        quantizers = llama_sequential(model, dataloader, method_name, DEV)
        print(time.time() - tick)


    ppl_eval(model, tokenizer, datasets=['wikitext2', 'ptb', 'c4'], model_seq_len=model.seqlen, batch_size=4, logger=logger)
    
    args.save = f"{args.output_dir}/{model_name}_{method_name}_{args.wbits}bit.pt"
