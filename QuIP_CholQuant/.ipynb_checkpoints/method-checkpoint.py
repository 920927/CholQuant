...
class QuantMethod:
    ...
    def preproc(self, preproc_gptqH=False, percdamp=.01, method=None,
        ...
        if method == 'cholquant':
            ...
            chol_diag = torch.linalg.cholesky(H)
            chol_diag = torch.diag(chol_diag)
            chol_diag  = chol_diag.clamp(min=1e-8)
            S = chol_diag ** (1/4)
            S_inv       = 1.0 / S

            w_q = w * S.unsqueeze(0)
            H_q = H * (S_inv.unsqueeze(1) * S_inv.unsqueeze(0))

            self.layer.weight.data = w_q.to(self.layer.weight.data.dtype)
            self.H.data = H_q.to(self.H.data.dtype)
            self.s_inv = S_inv
