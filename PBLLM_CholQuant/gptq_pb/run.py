...
def get_model(model):
    ...
    tokenizer = LlamaTokenizer.from_pretrained(model, device_map="cpu", trust_remote_code=True)
...
@torch.no_grad()
def quant_sequential(args, model, dataloader, method, dev):
    print("Starting ...")
    ...
    for i in range(len(layers)):
        ...
        for name in gpts:
            print(i, name)
            print("Quantizing ...")
            info = gpts[name].fasterquant(name, args.low_frac, percdamp=args.percdamp, blocksize=args.blocksize, method=method, whitenq_n=args.whitenq_n
            )
            gpts[name].free()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--model', type=str, default="/ruisun2025/dx/llm_model/Llama-2-7b-hf",
    ...
    parser.add_argument(
        '--output_dir', type=str, default="./output_models",
        help='The folder path for saving the model and logs'
    )
    parser.add_argument(
        '--method', type=str, default="gptq", choices=['pbllm', 'q2n', 'cholquant']
    )
    parser.add_argument(
        '--whitenq_n', type=int, default=4    
    )
    parser.add_argument(
        "--low_quant_method",
        type=str,
    ...
    parser.add_argument('--save_path', type=str, default='', help='Save quantized checkpoint under this name.')
    parser.add_argument(
        "--save_dir",default=None, type=str, help="direction for saving fake quantization model"
    )
    parser.add_argument(
        "--disable_gptq",
        action="store_true",
...
#     save_file = "../output/" + save_title.replace("/", "_") + ".pt"
    ...
    log_filename = f"quant_{model_name}_{method_name}_{args.low_quant_method}_n{args.whitenq_n}.log"
    ...
    logger.info(f"开始任务。参数配置: {args}")
    
    
    if args.load_quantized:
        model, tokenizer = get_model(save_file)
        model.eval()
    elif args.low_frac:
        ...
        quant_sequential(args, model, dataloader, method_name, device)
...
#         model.save_pretrained(args.save_path)
    ...
    ppl_eval(model, tokenizer, datasets=['wikitext2', 'ptb', 'c4'], model_seq_len=model.seqlen, batch_size=4, logger=logger)
        
    args.save = f"{args.output_dir}/{model_name}_{method_name}_{args.low_quant_method}bit_n{args.whitenq_n}.pt"
