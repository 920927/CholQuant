...
import time
import itertools
# from dx_data_utils import get_test_data
...
@torch.no_grad()
def ppl_eval(model, tokenizer, datasets=['wikitext2', 'ptb', 'c4'], model_seq_len=2048, batch_size=32, device="cuda", logger=None):
    model.to(device)
    model.eval()
    ppls = {}
    ...
    logger.info(f"PPL after pruning: {ppls}")
