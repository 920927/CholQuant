...
class GPTQ:
    ...
    def fasterquant(
        self, name, blocksize=128, percdamp=.01, groupsize=-1, actorder=False, static_groups=False, args=None
    ):
        ...
        if args.method == 'cholquant':
            damp = 0.01 
            H.diagonal().add_(damp * torch.mean(H.diagonal()))
            H_diag_orig = torch.linalg.cholesky(H).float()
            H_diag_orig = torch.diag(H_diag_orig)
            
            H_diag  = H_diag_orig.clamp(min=1e-8)
            S = H_diag ** (1/args.whitenq_n)
            S_inv   = 1.0 / S
            S[dead]     = 1.0
            S_inv[dead] = 1.0
            ...
            H_s = H * (S_inv.unsqueeze(1) * S_inv.unsqueeze(0))
            self.quantizer.find_params(W_s)
            H, W = H_s, W_s
        ...
        if args.method == 'q2n' and name != 'mlp.down_proj':
            threshold, lambda_t = 0.1, 0.2
            ...
            index = fast_find_energy_drop_index(s, threshold=threshold) # 0.05      # 0.05,0.1;   0.1 0.1;  best: 0.1,0.2
        ...
        if args.method == 'whitenq':
            Q = Q * S_inv.unsqueeze(0)
...
@torch.no_grad()
def gptq_fwrd(args, model, dataloader, dev):
    '''
    From GPTQ repo 
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                gptq[name].fasterquant(name, 
                    percdamp=args.percdamp, groupsize=layer_w_groupsize, actorder=args.act_order, static_groups=False, args=args
                )
